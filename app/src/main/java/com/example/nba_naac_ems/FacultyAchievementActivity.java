...
public class FacultyAchievementActivity extends AppCompatActivity {

    Spinner typeSpinner, yearSpinner, deptSpinner;
    EditText otherType, name, empId, date, title, description;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_faculty_achievement);
        ...
        // Check for Edit Mode
        FacultyAchievement existing = (FacultyAchievement) getIntent().getSerializableExtra("achievement");
        if (existing != null) {
            achievementId = existing.id;
            formTitle.setText("Edit Faculty Achievement");
    ...
    private void setupSpinners() {
        // Achievement Types
        String[] types = {
                "Select Type", "PhD Awarded", "Publication", "Journal Paper", "Conference Paper",
                "Book Chapter", "Patent", "Research Project", "Funded Project", "Consultancy",
                "Workshop Conducted", "Faculty Development Program", "Seminar", "Guest Lecture",
                "Certification", "Award & Recognition", "Best Teacher Award", "State Level Award",
                "National Level Award", "International Award", "Professional Membership",
                "NPTEL Course", "Training Program", "Industrial Visit", "Other"
        };
        ...
        for (int i = 1999; i <= 2040; i++) years.add(i + "-" + (i + 1));
        ArrayAdapter<String> yearAdapter = new ArrayAdapter<>(this, R.layout.spinner_item, years);
        yearAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        yearSpinner.setAdapter(yearAdapter);

        // Department
        String[] depts = {"Select Department", "CSE", "ECE", "EEE", "MECH", "CIVIL", "IT", "MCA", "MBA", "S&H"};
    ...
    private void populateFields(FacultyAchievement item) {
        name.setText(item.name);
        empId.setText(item.empId);
    ...
    private void saveAchievement() {
        ...
        String sEmpId = empId.getText().toString();
        ...
        if (sName.isEmpty() || sEmpId.isEmpty() || sYear.equals("Select Year") || sDept.equals("Select Department") || sDate.isEmpty() || sTitle.isEmpty() || sType.equals("Select Type")) {
        ...
        FacultyAchievement data = new FacultyAchievement(achievementId, sName, sEmpId, sYear, sTitle, sDept, sType, sDesc, sDate);
